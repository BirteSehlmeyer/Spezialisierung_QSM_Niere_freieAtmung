...
# Definition des Pfads und der Ausführungsdatei bse.exe
program_directory = "C:\\Program Files\\BrainSuite19a\\bin"
program_name = "bse"

# ändert das Arbeitsverzeichnis --> stellt sicher, dass das Programm im richtigen Verzeichnis ausgeführt wird 
os.chdir(program_directory)


# Aufbau des vollständigen Kommandos
command = f'{program_name} -i "{input_magnitude}" -o "{output_mask}" -r 1 -d {diffusion_value} -s {edge_values} -p'

# Falls bse nicht gefunden wird, können folgende Befehle verwendet werden:
# command = '"C:\\Program Files\\BrainSuite19a\\bin\\bse" -i ' + input_magnitude + ' -o ' + output_mask + ' -r 1 -d ' +  str(diffusion_value) + ' -s ' + str(edge_values) + ' -p'
# ggf, die Endung .exe 
# command = f'cd"{program_directory}" && "{program_name}" -i {input_magnitude} -o {output_mask} -r 1 -d {diffusion_value} -s {edge_values} -p'
